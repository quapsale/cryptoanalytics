...
btc = btc.rolling(window_size, on='Date').mean().sort_values(by='Date')
btc = btc.set_index('Date')
eth = group.get_group('eth')
eth = eth.rolling(window_size, on='Date').mean().sort_values(by='Date')
...
for coin in coins:
    ...
    subset = subset.rolling(window_size, on='Date').mean().sort_values(by='Date')
...
file_name_btc = os.path.join(corr_data_path, 'weekly_sw_btc_VOL.csv')
file_name_eth = os.path.join(corr_data_path, 'weekly_sw_eth_VOL.csv')
