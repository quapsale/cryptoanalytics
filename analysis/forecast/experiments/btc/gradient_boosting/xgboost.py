"""
File: xgboost.py
Description: Experiments with XGBoost model.
...
gb_path = './analysis/forecast/experiments/btc/gradient_boosting/'
...
logs_path = './analysis/forecast/experiments/btc/loggers/xgboost/metrics.csv'
...
# Save model
model.save_model(pretrained_path + 'xgboost' + version + '.txt')
...
np.savetxt(predictions_path + 'xgboost' + version + '.txt', test_pred['BTC'])
...
with open(metrics_path, 'a') as metrics:
    ...
    writer.writerow([now, 'xgboost', version, mse, rmse, mae, mape])
